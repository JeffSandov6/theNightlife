...
class partyPostsCells: UITableViewCell{
...
class partyEvents: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func getPosts() {
        ...
        ref?.child("partyPosts").observe(.childAdded, with: { (snapshot) in //also have to add a child deleted
    ...
    private func getPostWithId(postId: String) {
        ...
        let postReference = Database.database().reference().child("partyPosts").child(postId)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell:partyPostsCells = self.tableView.dequeueReusableCell(withIdentifier: cellReuseIdentifier) as! partyPostsCells
        
        let post = posts[indexPath.row]
        
        if let id = post.postId {
            
            let ref = Database.database().reference().child("partyPosts").child(id)
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) { //what will happen when we swipe to delete
        ...
        if let postId = post.postId {
            ...
            Database.database().reference().child("partyPosts").child(postId).removeValue(completionBlock: { (error, ref) in
